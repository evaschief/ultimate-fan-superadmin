...
import SchedulerConfigClient, { SchedulerConfig } from './SchedulerConfigClient';

const FALLBACK: SchedulerConfig = {
  maxOpenBets: 5, maxNewBetsPerUpdate: 1, pacingIntervalSeconds: 120,
...
export default async function BetSchedulerPage() {
  ...
  const legacyVersions = data?.value?.versions ?? [];
  const legacyCurrent = Array.isArray(legacyVersions) ? legacyVersions.find((version: any) => version.isCurrent)?.config : undefined;
  const current = normalize(data?.value?.config ?? legacyCurrent);
  return <div className="p-5 pb-10 max-w-4xl">
    <div className="mb-5"><h1 className="text-lg font-semibold text-gray-900">Bet Catalogue</h1><p className="text-sm text-secondary mt-1">These settings control the live bet scheduler.</p></div>
    ...
    <SchedulerConfigClient initialConfig={current} />
  </div>;
}
